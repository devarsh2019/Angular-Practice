...
@Component({
  ...
  standalone: false,
  templateUrl: './pipe-data.html',
  styleUrl: './pipe-data.scss',
})
export class PipeData {
  selectedIndex: number | null = null;
  displayDataRight = { title: '', description: '', color: '' };
  onClick(items: any, index: number) {
    this.selectedIndex = index;
    this.displayDataRight.title = items.name;
    this.displayDataRight.description = items.description;
    this.displayDataRight.color = items.color;
  }
  titleColor = 'black';

  onSaveParent(data: any) {
